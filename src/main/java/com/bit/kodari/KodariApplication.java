...
public class KodariApplication {
    ...
    private static CoinService coinService_static;
    private static RegisterCoinAlarmService registerCoinAlarmService_static;

    @PostConstruct
    private void initStatic() {
        coinService_static = this.coinService;
        registerCoinAlarmService_static = this.registerCoinAlarmService;
    }
...
//        upbitWebSocket.start(); // 업비트 웹 소켓 실행
        ...
        List<CoinDto.GetCoinRes> getCoinResBithumb = coinService_static.getCoinsByMarket(2);// 빗썸 전체 코인 심볼 조회

        for (int i = 0; i < getCoinResBithumb.size(); i++) {
            bithumbCoinSymbolSet.add(getCoinResBithumb.get(i).getSymbol());
        }

        bithumbWebSocket = new BithumbWebSocketListener(coinService_static, registerCoinAlarmService_static, bithumbCoinSymbolSet);
        bithumbWebSocket.start();// 웹 소켓 실행/**/


//        String symbol = "BTC";
//        //CoinService coinService = new CoinService();
//        List<CoinDto.GetCoinRes> getCoinRes = coinService_static.getMarketCoinBySymbol(2,symbol);
//        // marketIdx, coinIdx 로 RegisterCoinAlarm에서 사용자 지정가격 조회
//        int coinIdx = getCoinRes.get(0).getCoinIdx();
//        List<RegisterCoinAlarmDto.GetRegisterCoinAlarmRes> getRegisterCoinAlarmRes = registerCoinAlarmService_static.getRegisterCoinAlarmPriceByMarketIdxCoinIdx(2,coinIdx);
